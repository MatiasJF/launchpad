...
**2. Pick the section**, or add one — this file is young and its shape should follow what actually bites.

**3. Decide WHERE it belongs.** A finding about the chain, an SDK or the toolchain that would help *any* BSV
project goes in **`~/.claude/bsv-field-notes.md`**, which lives outside every repository and is loaded in all
of them. Only findings specific to this codebase belong in `docs/LESSONS.md`. Putting a general lesson in a
project file is how the next project rediscovers it.

**4. Write it symptom first.** The symptom is what the next person will have in front of them; the cause is
...
**5. Verify the claim before writing it down.** A wrong lesson is worse than none — it sends the next person
...
**6. Update the KB** — `docs/STATE.md` if it changes the current picture, `docs/DECISIONS.md` if a decision was made. `LESSONS.md` is the distilled layer, not a replacement for either.
