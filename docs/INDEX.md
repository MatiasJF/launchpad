...
- Stas curve SELL assembly (Step 3, ADR-028) · TX2 reserve refund = [pool SELL input 0xc1 + operator cosign + '51' selector, OPERATOR-funded fee input 0x41 consumed whole] → [reserve successor, seller refund] (2 outputs — the covenant pins exactly these; the atomic 3-output sell is infeasible, see DECISIONS ADR-028 step-3). Operator funds+cosigns; output-1 = the seller's recorded address at the curve refund (payee NOT covenant-bound = accepted operator-trust; FIX-3 seller-SIGHASH_ALL binding was refuted + reverted) → `packages/curve/src/stasSellAssembly.ts` (`buildStasSellRefundTx`); runtime sell-unlock encoder `packages/curve/src/curvePool.ts` (`encodeSellUnlockingHex`, byte-proven vs compiled ABI); offline sell + FIX-2 tests in `packages/curve/service/verify-stas.ts`
