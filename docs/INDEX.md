...
- Stas curve SELL assembly (Step 3, ADR-028) · TX2 reserve refund = [pool SELL input 0xc1 + operator cosign + '51' selector, operator fee input 0x41 consumed whole] → [reserve successor, seller refund] (2 outputs — the covenant pins exactly these; the atomic 3-output sell is infeasible, see DECISIONS ADR-028 step-3); validates covenant input via @bsv/sdk → `packages/curve/src/stasSellAssembly.ts` (`buildStasSellRefundTx`); runtime sell-unlock encoder `packages/curve/src/curvePool.ts` (`encodeSellUnlockingHex`, byte-proven vs compiled ABI); offline sell tests in `packages/curve/service/verify-stas.ts`
- Stas curve SELL server actions (Step 3; TX1 STAS return sequenced, TX2 operator reserve-refund cosign, pool advances at cosign, mirror recordCurveBuy guard) → `apps/web/lib/stas-actions.ts` (`prepareStasSell`, `recordStasSell`, `finalizeStasSell`)
- STAS back-to-genesis authenticity (operator runs BEFORE co-signing a sell refund; walks ancestry to the operator's own issuance, tail-fingerprint match, fail-closed) → `apps/web/lib/settle-actions.ts` (`verifyStasBackToGenesis`, `findStasOutputToPkh`)
