...
- **Consequences:** buys are concurrent and cheap; settlement is a serialized/batched backend concern that never risks funds. **Known follow-ups:** (a) buyer currently *pays before* `placeOrder`, so a rejected (oversold) order means a paid buyer needing a refund — correct fix is **reserve-then-pay** with reservation expiry; (b) a hard browser crash mid-settle leaves an order stuck in `settling` — needs a **stale-lock sweep** (release `settling` older than N minutes); (c) the Postgres migration should replace the sum-in-transaction guard with an atomic `sold` counter (`UPDATE … WHERE sold+n<=cap`) or SERIALIZABLE isolation, since READ COMMITTED could let two aggregates race.
